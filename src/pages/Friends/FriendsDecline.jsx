    ...
    const fetchFriends = async () => {
      try {
        const response = await axiosInstance.get(
          `/friendship/account/${accountId}/status?status=Declined&page=${currentPage}&size=${pageSize}`,
        );
        ...
        setError("Có lỗi xảy ra khi tải danh sách từ chối");
  ...
  return (
    <div className="min-h-screen bg-gray-100 px-5 pt-7">
      <h3 className="mb-5 text-xl font-bold text-gray-800">
        Danh sách bạn bè đã từ chối kết bạn
      </h3>
      {friends.length === 0 ? (
        <div className="xl:grid-cols-5flex grid w-full grid-cols-1 items-center justify-center gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <p className="text-gray-500">
            Không có lời từ chối nào trong danh sách để hiển thị
          </p>
        </div>
      ) : (
        <>
