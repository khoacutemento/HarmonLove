    ...
    const fetchFriends = async () => {
      try {
        const response = await axiosInstance.get(
          `/friendship/account/${accountId}/status?status=Block&page=${currentPage}&size=${pageSize}`,
        );
        ...
        setError("Có lỗi xảy ra khi tải danh sách chặn");
      ...
      {friends.length === 0 ? (
        <div className="xl:grid-cols-5flex grid w-full grid-cols-1 items-center justify-center gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <p className="text-gray-500">
            Không có bạn trong danh sách chặn để hiển thị
          </p>
        </div>
      ) : (
        <>
